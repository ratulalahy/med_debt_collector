...
@app.post("/verify_resident_tool")
async def verify_resident_tool(request: RetellFunctionRequest):
    """
    Handle Retell custom function call for resident verification.
    Verifies resident details against database using fuzzy name matching and DOB comparison.
    Returns verification status with patient details if successful.
    """
    logger.info(f"Received Retell function call: {request.name}")
    
    try:
        # Extract arguments from Retell's args dictionary
        args = request.args
        resident_fname = args.get("resident_fname", "").strip()
        resident_lname = args.get("resident_lname", "").strip()
        resident_dob = args.get("resident_dob", "").strip()
        resident_id = args.get("resident_id", "").strip()
        
        # Validate required fields
        if not all([resident_fname, resident_lname, resident_dob, resident_id]):
            raise HTTPException(
                status_code=400,
                detail="Missing required fields: resident_fname, resident_lname, resident_dob, or resident_id"
            )
        ...
        provided_fname = resident_fname.lower()
        provided_lname = resident_lname.lower()
        provided_dob = resident_dob
        ...
        try:
            ...
            logger.warning(f"Invalid DOB format - provided: {provided_dob}, stored: {stored_dob}. Error: {str(e)}")
            dob_match = False
        ...
        result = {
            ...
            "message": "Verification successful" if is_verified else (
                f"Verification failed: {'Incorrect name' if not name_match else ''}"
                f"{' and ' if not name_match and not dob_match else ''}"
                f"{'Incorrect DOB' if not dob_match else ''}"
            ),
            "details": {
        ...
        # Add additional patient data if verified
        if is_verified:
            result.update({
        ...
        logger.info(f"Verification {'successful' if is_verified else 'failed'} for resident_id: {resident_id}")
        return result
        ...
        logger.error(f"Resident verification failed: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
